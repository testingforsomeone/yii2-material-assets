...
class Menu extends \yii\widgets\Menu
{
    ...
    public $submenuTemplate = "\n<div class='collapse' id='collapse Example' {show}>\n<ul  class='nav'>\n{items}\n</ul>\n</div>\n";
    public $activateParents = true;
    public $defaultIconHtml = '<i class="material-icons" style="line-height: 30px;font-size: 24px">trip_origin</i>';
    ...
    {
        if (isset($item['items'])) {
            $labelTemplate = '<a class="nav-link" data-toggle="collapse" href="#collapse Example" aria-expanded="false" aria-controls="collapse Example">{icon} {label}</a>';
            $linkTemplate = '<a class="nav-link" data-toggle="collapse" href="#collapse Example" aria-expanded="false" aria-controls="collapse Example">{icon} {label} </a>';
            $item['label'] = '<span>'.$item['label'].'<b class="caret"></b></span>';
        } else {
        ...
        $replacements = [
            ...
            '{icon}' => empty($item['icon']) ? $this->defaultIconHtml
                : '<i class="material-icons" style="line-height: 30px;font-size: 24px">' . $item['icon'] . '</i> ',
    ...
    protected function renderItems($items)
    {
        ...
        foreach ($items as $i => $item) {
            ...
            if (!empty($item['items'])) {
                ...
                $menu = str_replace("collapse Example", $i.$item['label'], $menu);
